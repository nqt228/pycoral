...
from detect_yolov5 import get_objects 
...
def draw_object(draw, obj, count, img_size):
...
def main():
  ...
  f_fps = open("fps_yolo5s.txt","w")
  fps = []
  size = []
  for img in os.listdir(directory):
  # Open image.
    # f = open("vallllllllllll/{}.txt".format(img[:-4]), "w")
    img_name = img 
    ...
    # np_img /= 255.0
    # np_img = (np_img/input_scale) + input_zero
    # np_img = np_img[np.newaxis].astype(np.uint8)
    ...
    for tile_size in tile_sizes:
      for tile_location in tiles_location_gen(img_size, tile_size,
        ...
        #print('objssssss',objs)
        for obj in objs:
          bbox = [obj.bbox.xmin, obj.bbox.ymin, obj.bbox.xmax, obj.bbox.ymax]
          #bbox = [obj.bbox.ymin, obj.bbox.xmin, obj.bbox.ymax, obj.bbox.xmax]
          bbox = reposition_bounding_box(bbox, tile_location)

          label = labels.get(obj.id, '')
          objects_by_label.setdefault(label,
                                      []).append(Object(label, obj.score, bbox))
    # #print('itemmmmmm',objects_by_label.items())
    # tile = img
    # _, scale = common.set_resized_input(interpreter, tile.size,lambda size, img=tile: img.resize(size, Image.NEAREST))
    # interpreter.invoke()
    # objs = get_objects(interpreter, args.score_threshold, scale)
    ...
    for label, objects in objects_by_label.items():
      ...
      for idx in idxs:
        obj = objects[idx]
        # print(round(obj.score,2))
        # f.write('{} {} {} {} {} {}\n'.format(0,round(obj.score,2),obj.bbox[0],obj.bbox[1],obj.bbox[2],obj.bbox[3]))
        draw_object(draw, objects[idx],count, img_size)

    print('fps: ',1/(time.time()-start))
    fps_per_img = 1/(time.time()-start)
    fps.append(fps_per_img)

    # img.show()
    # f.close()
  fps = mean(fps)
  size = mean(size)
  f_fps.write('Mean fps : {}'.format(fps))
  f_fps.write('Average Number pix: {}'.format(size))
  f_fps.close()
  if args.output:
