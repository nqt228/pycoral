...
class EdgeTPUModel:
    ...
    def __init__(self, model_file, conf_thresh=0.25, iou_thresh=0.45, filter_classes=None, agnostic_nms=False, max_det=1000):
        """
    ...
    def forward(self, x:np.ndarray, with_nms=True) -> np.ndarray:
        ...
        # else:    
        result = result[0]
        bbox, confidence, classes = result[:,:4] , result[:,4:5], 0
        bbox = xywh2xyxy(bbox)
        
        return bbox, confidence, classes
